...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (body.operation === "upload") {
      ...
      if (body.fileB64.length > Math.ceil(MAX_BYTES * 4 / 3) + 8) {
      ...
      if (caller.role === "admin") {
        if (["superadmin", "admin"].includes(target.role)) {
          return errorResponse(req, 403, "Only a superadmin can attach documents to administrator accounts");
        }
        if (caller.branch_id !== target.branch_id) {
      ...
      try {
        bytes = b64ToBytes(body.fileB64);
      } catch {
        return errorResponse(req, 400, "Invalid file encoding");
      }
