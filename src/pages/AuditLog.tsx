...
interface LogRow {
  ...
  eventSource: 'legacy_client' | 'database' | 'edge';
}
...
const PAGE_SIZE = 50;
const tables = ['users', 'staff', 'students', 'branches', 'classes', 'class_enrollments', 'attendance', 'grade_entries', 'books', 'book_borrowings', 'student_fees', 'donors', 'grants', 'grant_transactions', 'transactions', 'messages', 'surveys', 'survey_branch_submissions', 'survey_respondents', 'survey_questions', 'survey_branch_responses', 'survey_individual_responses', 'organization_settings', 'user_documents', 'parent_student_links'].sort();
...
export default function AuditLog() {
  ...
  const load = useCallback(async () => {
    ...
    let q = supabase
      .from('activity_logs')
      .select(`
        id, action_type, table_name, description, created_at, event_source,
    ...
    setLoading(false);
    if (error) { setLoadFailed(true); toast.error(t('common.loadFailed')); return; }
    ...
    const mapped: LogRow[] = ((data ?? []) as any[]).map((r) => ({
      ...
      eventSource: r.event_source ?? 'legacy_client',
    }));

    setRows(mapped);
  }, [page, dateFrom, dateTo, filterTable, filterAction, search, t]);
    ...
    <div className="mx-auto max-w-7xl space-y-5 p-0 sm:p-2 lg:p-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            {t('common.recordedHistory', { count: total })}
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => void exportExcel()} disabled={!hasModuleAccess('auditLog', 'export') || loading || loadFailed || exporting || rows.length === 0} className="gap-1.5">
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <div className="space-y-1">
          ...
          <Input type="date" aria-label={t('common.from')} value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="w-38 h-9" />
        </div>
        <div className="space-y-1">
          <p className="text-xs font-medium text-muted-foreground">To</p>
          <Input type="date" aria-label={t('common.to')} value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="w-38 h-9" />
        </div>
        <Select value={filterTable} onValueChange={setFilterTable}>
          <SelectTrigger aria-label={t('common.auditTableFilter')} className="w-44 h-9"><SelectValue placeholder="All tables" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={filterAction} onValueChange={setFilterAction}>
          <SelectTrigger aria-label={t('common.auditActionFilter')} className="w-44 h-9"><SelectValue placeholder="All actions" /></SelectTrigger>
          <SelectContent>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div role="alert" className="p-8 text-center">
              ...
              <Button variant="outline" className="mt-4" onClick={() => void load()}>{t('common.retry')}</Button>
            </div>
          ) : rows.length === 0 ? (
            ...
            <div className="overflow-x-auto" tabIndex={0} role="region" aria-label={t('nav.auditLog')}>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/40">
                    <th className="text-start px-4 py-3 font-medium text-muted-foreground whitespace-nowrap">Timestamp</th>
                    <th className="text-start px-4 py-3 font-medium text-muted-foreground">Actor</th>
                    <th className="text-start px-4 py-3 font-medium text-muted-foreground">Action</th>
                    <th className="text-start px-4 py-3 font-medium text-muted-foreground">Table</th>
                  ...
                  {rows.map((row, i) => (
                    <tr key={row.id} className={`border-b last:border-0 ${i % 2 === 0 ? '' : 'bg-muted/20'}`}>
                      ...
                      <td className="px-4 py-2.5 max-w-md text-muted-foreground text-xs">
                        {row.description ?? '—'}
                        <p className="mt-1">{t(row.eventSource === 'legacy_client' ? 'common.legacyHistory' : 'common.verifiedHistory')}</p>
                      </td>
                    </tr>
                  ))}
                </tbody>
