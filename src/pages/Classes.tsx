...
import { Plus, MoveHorizontal as MoreHorizontal, Users, Clock, MapPin, Pencil, Trash2, Calendar, BookOpen, GraduationCap, Building2, Search, UserPlus, UserMinus, Loader2, Power, PowerOff, Archive } from 'lucide-react';
...
export function Classes() {
  ...
  const handleStatusChange = async (classId: string, status: 'active' | 'inactive' | 'archived') => {
    const res = await updateClass(classId, { status });
    if (res.success) {
                  ...
                  <div className="flex items-center gap-1 ml-2 shrink-0">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuSeparator />
                        {cls.status !== 'active' && (
                          <DropdownMenuItem onClick={() => handleStatusChange(cls.id, 'active')}>
                            <Power className="mr-2 h-4 w-4 text-emerald-500" />
                            Set Active
                          </DropdownMenuItem>
                        )}
                        {cls.status !== 'inactive' && (
                          <DropdownMenuItem onClick={() => handleStatusChange(cls.id, 'inactive')}>
                            <PowerOff className="mr-2 h-4 w-4 text-amber-500" />
                            Set Inactive
                          </DropdownMenuItem>
                        )}
                        {cls.status !== 'archived' && (
                          <DropdownMenuItem onClick={() => handleStatusChange(cls.id, 'archived')}>
          ...
          <div className="space-y-5 py-2">
            ...
            {editingClass && (
              <div className="space-y-1.5">
                ...
                <Select
                  value={form.status}
                  onValueChange={(v) => setForm((p) => ({ ...p, status: v as 'active' | 'inactive' | 'archived' }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
