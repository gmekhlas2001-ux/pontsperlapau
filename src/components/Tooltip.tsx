...
export function Tooltip({ content, children, position = 'top' }: TooltipProps) {
  ...
  return (
    <div className="relative inline-block">
      ...
      >
        {children || <HelpCircle className="w-4 h-4 text-slate-400 hover:text-slate-600 transition-colors" />}
      </div>

      {isVisible && (
        <div className={`absolute z-50 ${positionClasses[position]} pointer-events-none`}>
          <div className="bg-slate-800 text-white text-sm px-3 py-2 rounded-lg shadow-lg max-w-xs whitespace-normal">
            {content}
