 ...
 *
 * The app uses a custom HMAC auth flow (no Supabase JWT) and keeps a validated
 * identity cache in sessionStorage, or localStorage when “Remember me” is
 * selected. Service-layer code calls
...
/**
 * Read the current user's scope from the validated browser-session cache.
 *
 ...
 *
 * This function is intentionally synchronous — Web Storage access is fast
 * and avoids async overhead in hot paths like query builders.
 */
export function getCurrentScope(): CurrentScope {
  // SSR guard: Web Storage is unavailable in server-rendered contexts.
