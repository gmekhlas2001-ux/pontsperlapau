 ...
 * Settings service.
 *
 * Reads and writes the `organization_settings` key-value table, exposes
 * password change (delegates hash to the update-user edge function),
 * role/permission listing, activity log retrieval, CSV export, and system stats.
 *
 * All setting values are stored as text in the DB; numeric/boolean values are
 * serialised by the caller and parsed on the way out.
 */
...
/**
 * Change the current user's password. Validates the current password and
 * delegates the hash + write to the update-user edge function — anon can
 * no longer call hash_password directly or write password_hash.
 */
...
export async function exportTableAsCSV(tableName: 'users' | 'staff' | 'students' | 'classes' | 'books'): Promise<{ success: boolean; csv?: string; error?: string }> {
  try {
    ...
    const csvCell = (value: unknown) => {
      ...
      // values as formulas when the CSV is opened.
      if (/^[=+\-@]/.test(text)) text = `'${text}`;
      return /[",\r\n]/.test(text) ? `"${text.replace(/"/g, '""')}"` : text;
    };

    const headers = Object.keys(query[0]).map(csvCell).join(',');
