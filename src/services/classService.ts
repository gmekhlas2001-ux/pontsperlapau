 ...
 *
 * Handles CRUD for classes, teacher lookups, and student enrollment.
 * All list queries are branch-scoped via `scopedBranchId()` so admins only
 * see their own branch while superadmins see everything.
 * Deletes are soft (sets `deleted_at`) — hard purges must be done via DB tooling.
 */
